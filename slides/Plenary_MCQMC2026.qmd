...
More than one way to transform <br>an integral with respect to the multivariate Gaussian distribution into <br>an integral with respect to the standard uniform distribution

:::

::: {.column .tree-col width="25%"}
...
- This example is simple, but not overly simple

- The cost of function values is small compared to many realistic applications

- The sample size may be unreasonably large

- This example is meant to be illustrative, not definitive

- We do not discuss
  - How to choose the covariance kernel (Matèrn $3/2$ in this case) of the hyperparameters
  - The effect of choosing different $t_1, \ldots, t_d$ or quadrature weights (trapezpoidal rule)


# QMC is impressive, but shouldn't it be more impressive?
...
- Shouldn't scrambled Sobol'  give $\Order(n^{-3/2+\delta})$ convergence [{{< meta papers.owen_1997_scrambled_net_variance.cite >}}]({{< meta papers.owen_1997_scrambled_net_variance.doi_url >}})?

- We see $\Order(n^{-1+\delta})$ convergence
...
![](../notebooks/artifacts/figures/simple_sd_uniform_poly_d1.png){width=70%}

:::

::: {.column width="50%" style="text-align:center;"}

![](../notebooks/artifacts/figures/simple_sd_uniform_poly_d64.png){width=70%}
...
- After importance sampling, no longer additive
