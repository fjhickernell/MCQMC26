...
- $\Order(d N M)$ for [Kronecker]{.alert} <br>where $M$ size of the candidate set (do we need a better one?)
...
No universally best low discrepancy sequence
...
\end{equation*}
is not a practical [error bounds]{.alert} because 

- $\lVert f \rVert_{\cf}$ not known

- $\Dsc\bigl(\{\vx_i\}_{i=0}^{n-1}\bigr)$ depends on $\cf$ and may cost $\Order(dn^2)$ to compute

::: {.fragment style="margin-top:-1em;"}

<h3>Data-driven error bounds for QMC—_all make assumptions_</h3>

- $R$ [random replications]{.alert} with a Student's $t$, Berry-Esseen, bootstrap, \ldots confidence interval 

    [$\qquad \Order(R n)$ cost, $R$ vs. $n$ tradeoff, assume $R$ is big enough]{style="font-size:0.9em;"}
