...
- [Noisy]{.alert} data
  ...
  - $g : [0,1] \to \reals$ [unknown]{.alert}, modeled by Gaussian process

- Quantity of interest [(QOI)]{.alert}
  $\displaystyle \Ex\bigl[H(J(g)-a)\mid \vy \bigr]$

  - [Local loss]{.alert} $L : \reals \to [0,\infty)$

  - [Spatial risk/penalty]{.alert} $J(g)= \int_0^1 L(g(t)) \, \dif t$

  - [Outer penalty]{.alert} $H : [0,\infty) \to [0,\infty)$
...
- Quantity of interest [(QOI)]{.alert}
  ...
  - [Spatial risk/penalty]{.alert} $$J(g)= \int_0^1 L(g(t)) \, \dif t \approx J_d(\vg)
  :=
  \sum_{j=1}^d w_j L(g(t_j)),
  \quad
  \vg=(g(t_1),\dots,g(t_d))
  $$
...
- [Posterior]{.alert} distribution
  $\displaystyle \vg \mid \vy \sim \Norm(\vmu_d,\mSigma_d)$

  $$
  \Ex\bigl[H(J(g)-a)\mid \vy \bigr]
  \approx
  \int_{\reals^d}
  H(J_d(\vg)-a)
