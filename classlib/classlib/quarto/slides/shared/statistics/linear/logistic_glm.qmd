...
Admissions are not based on GRE alone.  A more realistic model allows the admission probability to depend on both GRE and GPA [(this is how the data were simulated to begin with)]{.alert}:
...
```{python}
df.loc[:, ["GRE", "GPA", "admit"]].sample(12, random_state=563).sort_values(["GRE", "GPA"])
```
...
|--------------|-------------|-------------------|
| OLS | $\Norm(\mu, \sigma^2)$ | Identity: $g(\mu) = \mu$ |
| Logistic regression | $\Bern(p(\vx))$ | Logit: $g(p) = \log\left(\dfrac{p}{1-p}\right)$ |
| Poisson regression | $\Pois(\mu(\vx))$ | Log: $g(\mu) = \log (\mu)$ |
